...
A public token metadata registry is available at [tapyrus-token-registry](https://github.com/chaintope/tapyrus-token-registry/).

To register token metadata:
1. Create an Issue in the repository with the following information:
   * Choose Network
   * Color ID
   * Payment base public key
   * Metadata JSON
2. The registry will verify that the P2C address derived from the payment base and metadata matches the Color ID
3. If verification passes, the metadata will be registered and publicly accessible

This registry provides a decentralized way for token issuers to publish their metadata and for wallets/applications to discover token information.
