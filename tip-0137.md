...
Validation consists of the following steps:
  ...
  2. Confirm the `header` satisfies the conditions.
       ...
       3. `jwk`.`keys` is an array that has only element, and the element satisfieds the conditions:
          ...
          3. `use` is `sig`
          4. `alg` is `ES256K`
          5. The `x` and `y` coordinates can be recovered as a point on the elliptic curve specified by the `crv` element, which in this case is the secp256k1 curve. And the P2PKH or CP2PKH address derived from the the point is the same as the `address` in the `payload`
