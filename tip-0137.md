...
Validation consists of the following steps:
  ...
  2. Confirm the `header` satisfies the conditions.
       ...
       3. `jwk`.`keys` is an array that has only element, and the element satisfieds the conditions:
          ...
          3. `use` is `sig`
          4. `alg` is `ES256K`
          5. `x` and `y` can be recovered as points on an elliptic curve, and the P2PKH or CP2PKH address derived from the the point is the same as the `address` in the `payload`
